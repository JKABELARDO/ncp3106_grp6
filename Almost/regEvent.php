        ...
        <div class="col-md-12">
            ...
            if (isset($_GET['studNumber'])) {
                ...
                if (mysqli_num_rows($query_run) > 0) {
                    ...
                    $insertQuery = "INSERT INTO attendees (lastName, firstName, middleName, studNumber, timeIn, chooseEvent) 
                                    VALUES ('" . $row['lastName'] . "', '" . $row['firstName'] . "', '" . $row['middleName'] . "', '" . $row['studNumber'] . "', '$currentTime', '$chooseEvent')";
                    $insertResult = mysqli_query($con, $insertQuery);

                    if ($insertResult) {
                        echo '<div class="alert alert-success" role="alert">Registered Successfully</div>';
                    } else {
                        echo '<div class="alert alert-danger" role="alert">Error in registration: ' . mysqli_error($con) . '</div>';
                    }
                } else {
